...
const updateRibbonFundedAppraisalAmount = () => {
  ...
  if (appraisedValue >= offerPrice) {
    ...
    result > 0 ? ribbonFundedHide.show() : ribbonFundedHide.hide();
...
const updateAppraisalGapAmount = () => {
  ...
  const offerPrice = parseInt(ribbonOfferPriceInput.val().replace(/\D/g, ""));
  const appraisal = parseInt(ribbonAppraisedValueInput.val().replace(/\D/g, "")); //
  //Appraisal Gap Amount = (Offer Amount OR Max Ribbon Value...whichever is LOWEST) -
  // (Appraised Value OR Ribbon Value...whichever is HIGHEST)

  // TODO: NEW
  const firstSummand = offerPrice < ribbonMaxValue ? offerPrice : ribbonMaxValue;
  const secondSummand = appraisal > ribbonValue ? appraisal : ribbonValue;
  const result = Math.floor(firstSummand - secondSummand);
  console.log(firstSummand, secondSummand, result);
  ribbonAppraisalGapAmount.text("$" + updateDigits(result));
  updateCap(result);

  // if (offerPrice > ribbonMaxValue) {
  //   const result = Math.floor(ribbonMaxValue - ribbonValue);
  //   // appraisalProtectionValue = result; //TODO: NEW
  //   ribbonAppraisalGapAmount.text("$" + updateDigits(result));
  //   updateCap(result);
  // } else if (offerPrice <= ribbonMaxValue) {
  //   const result = Math.floor(offerPrice - ribbonValue);
  //   ribbonAppraisalGapAmount.text("$" + updateDigits(result));
  //   // appraisalProtectionValue = result; //TODO: NEW
  //   updateCap(result);
  // }
...
const edgeCase = () => {
  //Appraised Value > Ribbon Value, then Ribbon Funded would be $0, but we still show all steps and calculate Gap?
  const appraisal = parseInt(ribbonAppraisedValueInput.val().replace(/\D/g, "")); //
  const ribbonValue = parseInt(ribbonValueInput.val().replace(/\D/g, ""));
  // TODO: NEW CODE
  //question here
  //
  if (appraisal >= ribbonValue && ribbonFundedAP.text() == "$0" && ribbonAppraisalGapAmountId.text() !== "$0") {
    console.log("edge case, showing all cards");
    $($(".pricing-wrap")[1]).show();
    $($(".pricing-wrap")[2]).show();
    $($(".pricing-wrap")[3]).show();
  }
...
const showAndHideBoxes = () => {
  //showHide apparaisal Positive Message

  if (ribbonAppraisalGapAmountId.text() == "$0") {
    console.log(123);
    $(".appraisedshow").show();
    $(".appraisedhide").hide();
  }
  //     $($(".pricing-wrap")[1]).find(".overlay-state").hide();
  //2
  const offerPrice = parseInt(ribbonOfferPriceInput.val().replace(/\D/g, ""));
  const ribbonMaxValue = parseInt(ribbonMaxValueInput.val().replace(/\D/g, ""));
  ...
  if (
    ribbonValue < 150000 ||
    ribbonValue > 1e6 ||
    "$0" == ribbonValueInput.val() ||
    "$" == ribbonValueInput.val() ||
    // TODO: NEW
    ribbonAppraisalGapAmountId.text() == "$0" ||
    "$" == ribbonAppraisalGapAmountId.text() ||
    ribbonFundedAP.text() == "$0" ||
    ribbonFundedAP.text() == "$"
    // TODO: END OF NEW
  ) {
    console.log("Hiding script 2");
    $($(".pricing-wrap")[1]).show();
    $($(".pricing-wrap")[2]).hide();
    $($(".pricing-wrap")[3]).hide();
    // TODO: NEW
    ...
    //   ribbonFundedAP.text() == "$"
    // ) {
    //   console.log("ribbon funded 1", ribbonFundedAP.text(), ribbonAppraisalGapAmountId.text());

    //   console.log("ribbon funded 2", ribbonFundedAP.text());
    //   $(".appraisedshow").show();
    //   $(".appraisedhide").hide();
    // }
  ...
  console.log("showing all boxes");
  $($(".pricing-wrap")[1]).show();
  $($(".pricing-wrap")[2]).show();
  $($(".pricing-wrap")[3]).show();
  //TODO: NEW CODE
  //initial state - do not remove
  $(".appraisedshow").hide();
  $(".appraisedhide").show();
};
